...
public class MemuHelper {
    ...
    public static List<JSONObject> bulid(List<Permission> treeNodes) {
        List<JSONObject> meuns = null;
        if (Collections.isEmpty(treeNodes)) {
        ...
        if(treeNodes.size() == 1) {
            ...
            meuns = toMenuList(oneMenuList);
        } else {
            meuns = toMenuList(treeNodes);
        }
        return meuns;
    }

    private static List<JSONObject> toMenuList(List<Permission> treeNodes) {
